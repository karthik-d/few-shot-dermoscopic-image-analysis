...
class PH2_Dataset(Dataset):
    ...
    def __init__(self, root, mode='train', img_ext='jpg', transform=None, target_transform=None):
        ...
        - mode: Determines the CSV file to use (PH2_<mode>.csv)
    ...
    def get_class_ids(class_names):
        return [
            PH2_Dataset.class_id_map.get(x)
    ...
    def get_sparse_label(csv_df, idx):
        ...
        for classlabel, value in one_hot_label.items():
            if value == 1:
                return PH2_Dataset.class_id_map.get(classlabel)
        return -1
